...
public suspend fun <T : Any> LazyPagingItems<T>.retryAppendPrependErrors(
...
public suspend fun <T : Any> LazyPagingItems<T>.retryAppendErrors(
    lazyGridState: LazyGridState,
    attemptDelayMillis: Long = ATTEMPT_DELAY_MILLIS,
) {
...
@OptIn(ExperimentalCoroutinesApi::class)
public suspend fun <T : Any> LazyPagingItems<T>.retryPrependErrors(
    lazyGridState: LazyGridState,
    attemptDelayMillis: Long = ATTEMPT_DELAY_MILLIS,
) {
    var lastAttemptTimestamp = 0L
